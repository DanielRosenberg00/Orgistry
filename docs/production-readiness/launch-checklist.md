...
| LC-1.5 | Add dependency/secret/SAST scanning to CI | ORG-PR-020 | 21 | CI runs scanners | SecEng | Open — advanced (S21): audit gates (`security.yml`), Gitleaks, CodeQL, Dependabot all configured + locally validated where runnable; closure on first green remote run + seeded-finding failure proof |
| LC-1.6 | Build deployable non-root artifacts + pipeline to staging | ORG-PR-001 | 22 | tagged build deploys to staging | Eng/Ops | Open |
| LC-1.7 | Security headers + `trustProxy` + DB timeouts | ORG-PR-011, 010, 021 | 18/22 | header/proxy/timeout tests | Eng | Open — advanced (Sprint 19): security headers on every response (`apps/api/src/plugins/security-headers.ts`) and typed `TRUST_PROXY` shipped with tests (ORG-PR-010/011 closed); DB/pool/statement timeouts (ORG-PR-021) remain for Sprint 22 |

## Stage 2 — Mandatory before production data

| ID | Action | Findings | Sprint | Evidence to close | Owner | Status |
| --- | --- | --- | --- | --- | --- | --- |
| LC-2.1 | **Backups + PITR configured** | ORG-PR-005 | 23 | backup job runs; PITR verified | Ops | Open |
| LC-2.2 | **Backup restore drill executed & passing** (mandatory) | ORG-PR-005, 028 | 23 | restore reconstructs DB to a timestamp + passes readiness/integration | Ops | Open |
| LC-2.3 | Secrets manager + rotation runbook | ORG-PR-006 | 22 | no committed secrets in non-local env; rehearsed rotation | Ops/SecEng | Open |
| LC-2.4 | Least-privilege DB roles (runtime vs migration) | ORG-PR-022 | 22 | runtime role cannot DDL | Ops | Open |
...
| LC-2.9 | Data-subject deletion/export path (per DG-3) | ORG-PR-025 | 23 | deletion/export integration tests | Eng/Legal | Open |
...
| LC-3.4 | Rate-limit fail-closed option + alerting | ORG-PR-009 | 18/24 | fail-closed test + alert | SecEng/Ops | Open — advanced (Sprint 19): fail-closed implemented, tested, and production-default (`RATE_LIMIT_FAILURE_MODE`; guard refuses `open` in production); the alerting half awaits ORG-PR-007 (Sprint 24) |
| LC-3.5 | Observability: metrics/tracing/dashboards/alerts | ORG-PR-007 | 24 | dashboard + synthetic-failure alert | Ops | Open |
| LC-3.6 | Incident process + production runbooks + ops docs | ORG-PR-008, 027 | 24 | tabletop passes; docs published | Ops | Open |
| LC-3.7 | External security review / pentest + DAST | ORG-PR-018, 020 | 25 | report with no unresolved high | SecEng | Open |
| LC-3.8 | Failure-injection + E2E + live SMTP CI | ORG-PR-026, 041, 044 | 25 | suites green | Eng | Open |
...
| LC-3.10 | Legal/privacy review (regime, retention, subprocessors, breach) | ORG-PR-025, 043 | 15/23 | legal sign-off | Legal | Open |

## Stage 4 — Mandatory shortly after launch

| ID | Action | Findings | Sprint | Evidence to close | Owner | Status |
| --- | --- | --- | --- | --- | --- | --- |
| LC-4.1 | PII retention enforcement in event metadata | ORG-PR-043, 015 | 23 | aged PII removed/anonymized | Eng/Legal | Open |
| LC-4.2 | JWT `kid`/rotation path | ORG-PR-049, 006 | 22+ | overlapping-key rotation test | SecEng | Open |
...
| LC-4.5 | Refresh subsystem doc refresh (stale docs) | ORG-PR-046 | 25 | docs match source | Eng | Open |
