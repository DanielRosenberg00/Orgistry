...
export interface CreateApiKeyParams {
  ...
  /** The clock instant used for the active-key (non-expired) count. */
  now: Date;
 ...
 * policy and does NOT shape HTTP responses. The entitlement/quota rules it
 * applies in the create transaction are mechanical: the ONE plan snapshot the
 * transaction resolves for itself supplies both the `api_keys_access` gate and
 * the `max_api_keys` ceiling.
 */
export interface ApiKeyRepository {
  /**
   * Create a key under `organizationId` and record `api_key.created`, with the
   * entitlement + quota decision enforced atomically: one transaction acquires
   * the organization's api-key-quota lock, resolves the CURRENT plan through
   * the same transaction (plan row `FOR SHARE`), re-checks `api_keys_access`
   * and the `max_api_keys` ceiling from that ONE coherent snapshot (a
   * pre-resolved limit could be stale — plan assignment is runtime-mutable),
   * counts ACTIVE keys (not revoked AND not expired at `now` — a revoked or
   * expired key never occupies a slot), rejects with `ENTITLEMENT_REQUIRED` /
   * `QUOTA_EXCEEDED`, then inserts. A failure writes nothing — the secret
   * hash is never persisted and the raw secret (generated by the caller) is
