...
5. **Verify your email (Sprint 16).** After registering, an advisory
   unverified-email banner appears in the shell and a verification email lands
   in Mailpit (<http://localhost:8025>). Open the message and follow the
   `…/auth/verify-email#token=…` link (the token rides in the URL fragment,
   which the browser never sends to any server): the page captures it,
   removes it from the URL, submits it to the backend in a POST body, and
   shows success; the banner disappears once the refreshed current
   user reports `emailVerified: true`. Try the link a second time to see the
   single-use rejection, and use **Resend email** in the banner to see resend
   invalidate the older link. Verification is advisory — nothing is blocked
   while unverified.
6. **Create / select an organization.** Use the org switcher to create a team
   organization (e.g. "Acme Corp"); you become its Owner. Switch between your
   personal workspace and the team org.
7. **View the overview.** The overview surfaces the selected org and your
   membership/role.
8. **Create a project.** On Projects, create one or more projects. On the **Free**
   plan you will hit `QUOTA_EXCEEDED` at `max_projects = 3` — a backend-authoritative
   error surfaced in the UI.
9. **Change the plan (optional).** On Plan & Entitlements, switch to **Pro** (demo
...
10. **Invite a user and view Mailpit.** On Invitations, invite an email address.
   Open Mailpit at <http://localhost:8025> to read the invitation email; the raw
   token appears only here (never in a URL or log).
11. **Create an API key and copy the one-time secret.** On API Keys, create a key
    with the `projects:read` scope. The raw secret is shown **once** — copy it now.
12. **Call the external read-only Projects API.** Use the secret as a bearer token
    ...
    the key.
13. **View the audit log.** On Audit, see the org action events generated by the
    steps above (project created, invitation created, API key created, plan
    changed), with metadata sanitized.
14. **Observe permission-aware UX and backend-authoritative errors.** Invite a
...
- The email-verification lifecycle end to end (issue → deliver → complete →
  single-use rejection → resend invalidation), advisory by policy.
