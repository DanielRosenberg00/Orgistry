...
| Resilience & recovery (ASVS V14 / SSDF PO.3 adjacent) | **Satisfied for the staging-like target (S25 capability, S28 programme — ORG-PR-005 closed)** | S25: repeatable logical backup with integrity checksum and provenance, restore drill into a fresh database reaching the packaged artifact, **PITR VERIFIED**, CI-gated, documented runbooks. S28 on the staging-like target: scheduled encrypted backups to **off-host DigitalOcean Spaces**, **continuous WAL archiving from the deployed database**, client-side AES-256-GCM, least-privilege backup role, recovery-point catalog, health checks that exit non-zero, artifact lifecycle, and **executed restore and PITR rehearsals recovering from that off-host storage, with measured staging-scale RPO/RTO** | single-region storage (Space and droplet both in `fra1`) so a regional outage is uncovered; no provider-managed PITR; no second storage provider; no alert routing on backup or archive health; no production-scale RPO/RTO | P2 | ORG-PR-028 |
...
| Operations — Environment Mgmt | ~2→3 | local runbook; backup/restore/PITR and retention runbooks with executed evidence (S25); **scheduled backup, WAL-shipping, health, and lifecycle jobs running on the staging-like target against off-host DigitalOcean Spaces, with operational runbooks executed against it (S28)** | no prod runbook; nothing schedules the RETENTION cleanup; single-region backups; no alert routing — ORG-PR-007/016/027 |
